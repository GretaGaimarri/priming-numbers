...
# Create a new DataFrame for opposite adjectives
adjectives_combined = pd.DataFrame({
    'adjective': adjectives['word'],  # Use the actual column name 'word'
    'opposite': adjectives['opposite'],  # Use the actual column name 'opposite'
    'bigsmall': adjectives['bigsmall']  # Assuming 'bigsmall' is the correct column
})
...
# Two adjectives display (one white, one black)
prime_white = visual.TextStim(win=win, name='prime_white', text='', font='Arial', pos=(0, 0.1), height=0.2, wrapWidth=None, ori=0,
                              color='white', colorSpace='rgb', opacity=1, languageStyle='LTR', depth=0.0)
prime_black = visual.TextStim(win=win, name='prime_black', text='', font='Arial', pos=(0, -0.1), height=0.2, wrapWidth=None, ori=0,
...
def run_trial(prime_text, opposite_text, bigsmall_value, congruent, reverse=False):
    ...
    # Prime display with two adjectives
    prime_white.setText(prime_text)
    prime_black.setText(opposite_text)
    prime_white.draw()
    prime_black.draw()
    win.flip()
...
for trial in trial_sequence:
    prime_text = trial['adjective']
    opposite_text = trial['opposite']
    big_small_value = trial['bigsmall']
    congruent = trial['congruent'] == 1
    response, reaction_time, accuracy = run_trial(prime_text, opposite_text, big_small_value, congruent)
    trial_data.append((prime_text, opposite_text, big_small_value, congruent, response[0], reaction_time, accuracy, 'first_run'))
...
for trial in trial_sequence_second_run:
    ...
    big_small_value = trial['bigsmall']
    congruent = trial['congruent'] == 1
    response, reaction_time, accuracy = run_trial(prime_text, opposite_text, big_small_value, congruent, reverse=True)
    trial_data.append((prime_text, opposite_text, big_small_value, congruent, response[0], reaction_time, accuracy, 'second_run'))

with open('data.csv', 'w') as data_file:
    data_file.write('prime,opposite,big_small,congruent,response,reaction_time,accuracy,run\n')
    for data_point in trial_data:
        data_file.write(f"{data_point[0]},{data_point[1]},{data_point[2]},{data_point[3]},{data_point[4]:.4f},{data_point[5]},{data_point[6]}\n")
